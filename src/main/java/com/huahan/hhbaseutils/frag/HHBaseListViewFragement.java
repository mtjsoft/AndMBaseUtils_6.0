 ...
 * @author yuan
 */
public abstract class HHBaseListViewFragement<T> extends HHBaseDataFragment implements OnRefreshListener, OnScrollListener, OnItemClickListener {
    /**
     * 是否加载更多
     */
    ...
    public View initView() {
        View view = View.inflate(getPageContext(), R.layout.hh_activity_listview, null);
    ...
    public void initValues() {
        if (getArguments() != null) {
            mIsLoadMore = getArguments().getBoolean(FLAG_LOAD_MORE, true);
            mRefresh = getArguments().getBoolean(FLAG_REFRESH, true);
        }
    }
    ...
    private void getListData() {
        getListDataInThread(mPageIndex, new HHCallBackImp<T>() {
            @Override
            public void onSuccess(T data) {
                mTempList = (List<T>) data;
