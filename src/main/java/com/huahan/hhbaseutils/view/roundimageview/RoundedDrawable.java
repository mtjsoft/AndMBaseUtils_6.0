...
package com.huahan.hhbaseutils.view.roundimageview;
...
public class RoundedDrawable extends Drawable 
{
  ...
  /**
   * 从一个Bitmap获取到一个RoundedDrawable对象
   * @param bitmap			drawable显示的bitmap图形
   * @return				如果Bitmap为null，返回null，否则的话调用new RoundedDrawable(Bitmap)构造方法构造一个Drawable对象
   */
  ...
  /**
   * 通过drawale对象来获取一个RoundedDrawable对象，如果获取失败的话，返回这个drawable本身
   * @param drawable
   * @return
   */
  public static Drawable fromDrawable(Drawable drawable)
  {
    if (drawable != null) 
    {
      ...
      {
        //如果drawable对象本身就是一个RoundedDrawable对象，就直接返回这个drawbale
        return drawable;
      } else if (drawable instanceof LayerDrawable) 
      {
    	//如果这个对象是一个LayerDrawable对象，则把每一层的drawable对象都设置为一个RoundedDrawable对象，并
    	//重新设置为LayerDrawable的每一层
        LayerDrawable ld = (LayerDrawable) drawable;
        int num = ld.getNumberOfLayers();
        // 遍历每一层，并重新设置每一层显示的drawable
        for (int i = 0; i < num; i++) 
        {
      ...
      //把一个drawable对象转换成Bitmap，如果Bitmap不为null的话，就通过该Bitmap来
      //实例化RoundedDrawable对象
  ...
  public static Bitmap drawableToBitmap(Drawable drawable) 
  {
    ...
    {
    	//如果drawable对象是一个BitmapDrawable的话，就直接获取bitmap
      return ((BitmapDrawable) drawable).getBitmap();
    }
    //创建一个空白的Bitmap，并且实例化一个Canvas对象，然后通过drawable把drawable对象绘制到
    //这个空白的Bitmap上
    Bitmap bitmap;
  ...
  {
	//根据当前的状态获取当前状态下应该显示的颜色值，用户绘制不同颜色的边框
    int newColor = mBorderColor.getColorForState(state, 0);
    //判断当前的颜色值和新的颜色值是否相同，如果不同的话设置为新的颜色值
  ...
  protected void onBoundsChange(@NonNull Rect bounds) 
  {
    ...
    //当drawable的大小发生改变的时候重新设置大小，并且更新绘制的矩阵
    mBounds.set(bounds);
    updateShaderMatrix();
  }
